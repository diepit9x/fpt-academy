...
public class AirplaneManagement {
	...
	do {
	    ...
	    case Constant.REMOVE_AIRPLANE: {
		System.out.println("****Remove airplane from airport****");
		System.out.print("airplaneId: ");
		airplaneId = scanner.nextLine();
		try {
		    boolean status = airportService.removeAirplane(airplaneId);
		    if (status) {
			System.out.println("Remove airplane from airport successfully");
		    }
		} catch (Exception e) {
		    System.err.println(e.getMessage());
		}
