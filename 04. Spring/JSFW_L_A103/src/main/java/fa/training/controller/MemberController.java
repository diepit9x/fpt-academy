...
public class MemberController {
    ...
    public ResponseEntity<ResponseObject> register(@Valid @ModelAttribute RegisterDTO registerDTO,
    ...
    @PostMapping("/login")
    public ResponseEntity<ResponseObject> login(HttpSession session, @Valid @ModelAttribute LoginDTO loginDTO,
	    BindingResult bindingResult) {
	Member memberSession = SessionUtil.getLoggedInUser(session);
	if (memberSession != null) {
	    return ResponseEntity.ok(ResponseObject.builder().status(400).object("You are logged In").build());
	}
	Map<String, String> errors = new HashMap<>();
	if (bindingResult.hasErrors()) {
	    bindingResult.getFieldErrors().forEach(error -> errors.put(error.getField(), error.getDefaultMessage()));
	...
	return ResponseEntity.ok(ResponseObject.builder().status(200).object("Login successfully").build());
    }
	...
	if (member != null) {
	    return ResponseEntity.ok(ResponseObject.builder().status(200).object(member).build());
	}
	...
	return ResponseEntity.ok(ResponseObject.builder().status(200).object("Update successfully").build());
    }
