...
public class MayDAO {
	...
	public SuDungMay findSDMByKH(KhachHang khachHang) {
		...
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			Query<SuDungMay> query = session.createQuery("FROM SuDungMay sdm WHERE sdm.khachHang = :khachHang AND sdm.thoiGianSuDung IS NULL", SuDungMay.class);
	...
	public boolean updateThoiGianSuDung(May may) {
		...
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			Query<SuDungMay> query = session.createQuery("FROM SuDungMay sdm WHERE sdm.may = :may AND sdm.thoiGianSuDung IS NULL", SuDungMay.class);
			query.setParameter("may", may);
			SuDungMay suDungMay = query.getSingleResultOrNull();
			if (suDungMay != null) {
