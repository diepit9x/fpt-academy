...
$(document).on("submit", "#tao-moi-khach-hang", function (e) {
  var isValid = true;
  const maKhachHang = $("#maKH").val();
  const tenKhachHang = $("#tenKH").val();
...
$(document).on("submit", "#dang-ki-su-dung-may", function (e) {
  ...
  //const thoiGianSuDung = $("#thoiGianSuDung").val();
...
//  }
 ...
 // if (!isInteger(thoiGianSuDung) || Number(thoiGianSuDung) <= 0) {
 //     alert("Thời gian sử dụng không hợp lệ");
 //     isValid = false;
 //   }
  if (!isValid) {
    e.preventDefault();
